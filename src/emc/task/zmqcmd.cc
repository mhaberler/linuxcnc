...
static int waitTicketCompleted(int ticket, double timeout)
{
	...
	fprintf(stderr, "update.ticket_update.cticket = %d status=%s\n",
		cticket, rcs_statename[status]);
...
int main (int argc, const char **argv)
{
   ...
   zmsg_t *reply = zmsg_recv (z_task);
    if (!reply) {
	fprintf(stderr, "%s:  z_submit: reply timeout\n", z_ident);
	z_finish();
	exit(1);
    }
    ...
    fprintf(stderr, "%s: got ticket = %d %s\n",
	    z_ident, ticket,  rcs_statename[response.task_reply().status()]);
