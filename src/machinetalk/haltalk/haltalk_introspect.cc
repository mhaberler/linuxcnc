...
int
process_describe(htself_t *self, const std::string &from,  zsock_t *socket)
{
...
int
describe_group(htself_t *self, const char *group, const std::string &from,  zsock_t *socket)
{
    int retval __attribute__((cleanup(halpr_autorelease_mutex)));
    rtapi_mutex_get(&(hal_data->mutex));
...
int
describe_comp(htself_t *self, const char *comp, const std::string &from,  zsock_t *socket)
{
    int retval __attribute__((cleanup(halpr_autorelease_mutex)));
    rtapi_mutex_get(&(hal_data->mutex));
