...
typedef int (*hal_group_callback_t) (hal_group_t *group,  void *arg);
typedef int (*hal_ring_callback_t)  (hal_ring_t *ring,  void *arg);
...
static inline int halpr_foreach_group(const char *name,
				      hal_group_callback_t callback,
				      void *arg)
{
    return halg_foreach_type(0, HAL_GROUP, name,
			     (hal_pertype_callback_t) callback,
			     arg);
}

static inline int halpr_foreach_ring(const char *name,
				      hal_ring_callback_t callback,
				      void *arg)
{
    return halg_foreach_type(0, HAL_RING, name,
			     (hal_pertype_callback_t) callback,
			     arg);
}
