...
MODULE_DESCRIPTION("Tiny Motion Controller for Machinekit");
MODULE_LICENSE("GPL");

int debug = 0;
RTAPI_MP_INT(debug, "debug motion");
...
static minimot_inst_t *minimot_array; // instance data array
static int comp_id;

static int export_minimot(const char *name, int num, minimot_inst_t *p);

static void update_minimot(void *arg, long l)
{
    minimot_inst_t *p = (minimot_inst_t *) arg;
...
int rtapi_app_main(void)
{
	...
	rtapi_print_msg(RTAPI_MSG_ERR, "%s: ERROR: invalid count: %d\n", name, count);
	return -1;
    }
    if ((comp_id = hal_init(name)) < 0) {
	rtapi_print_msg(RTAPI_MSG_ERR, "%s: ERROR: hal_init(%s) failed: %d\n",
			name, name, comp_id);
	return comp_id;
    }
    minimot_array = hal_malloc(count * sizeof(minimot_inst_t));
    if (minimot_array == 0) {
    ...
    for (i = 0; i < count; i++) {
	if ((retval = export_minimot(name, i, &minimot_array[i]))) {
	    rtapi_print_msg(RTAPI_MSG_ERR,"%s: ERROR: var export failed\n", name);
	    hal_exit(comp_id);
	    return -1;
	}
    }
...
void rtapi_app_exit(void)
{
    ...
    for (i = 0; i < count; i++) {
	minimot_inst_t *p = &minimot_array[i];
...
static int export_minimot(const char *comp, int n, minimot_inst_t *p)
{
    int retval;
    ...
    *(p->decodefail) = 0;
    ...
    if ((retval = hal_export_funct(name, update_minimot, &(minimot_array[n]),
				   1, 0, comp_id))) {
	rtapi_print_msg(RTAPI_MSG_ERR,
