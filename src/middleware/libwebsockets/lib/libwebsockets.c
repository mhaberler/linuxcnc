...
#ifndef LWS_USE_LIBEV
static const char *library_version = LWS_LIBRARY_VERSION " " LWS_BUILD_HASH;
#else
static const char *library_version = LWS_LIBRARY_VERSION " " LWS_BUILD_HASH " libev";
#endif /* LWS_USE_LIBEV */
...
libwebsocket_context_destroy(struct libwebsocket_context *context)
{
	 ...
	 * callbacks
     * pass protocol pointer as "in" parameter for "dispatcher" style callbacks
	 */

	while (protocol->callback) {
		protocol->callback(context, NULL, LWS_CALLBACK_PROTOCOL_DESTROY,
				NULL, (void*)protocol, 0);
		protocol++;
	}
    ...
    struct ev_loop *loop)
{
     ...
     */
    ev_io_init(
        w_accept, libwebsocket_accept_cb,
...
OpenSSL_verify_callback(int preverify_ok, X509_STORE_CTX *x509_ctx)
{
	...
	n = context->protocols[0].callback(context, NULL,
...
//#endif /* LWS_USE_LIBEV */
	...
	for (context->count_protocols = 0;
		 ...
		 * initialization if they want to
          * pass protocol pointer as "in" parameter for "dispatcher"
          * style callbacks
		 */
		info->protocols[context->count_protocols].callback(context,
			       NULL, LWS_CALLBACK_PROTOCOL_INIT, NULL,
                   (void*)&info->protocols[context->count_protocols], 0);
	}
