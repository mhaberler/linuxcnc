...
int pbvprintf( RTAPI_Message *msg, int level, const char *fmt, va_list ap)
{
    int modifier_l, code;
    Value *v;
...
int pbprintf(RTAPI_Message *msg, int level, const char *fmt, ...) {
    va_list ap;
    int result;
...
int main (int argc, char *argv[ ])
{
    ...
    if ((result = pb_encode(&stream, RTAPI_Message_fields, &msg))) {
	fwrite(buffer, 1, stream.bytes_written, stdout);
        exit(0);
    } else {
	fprintf(stderr, "rtprintf: pb_encode failed: %d - %s\n",
